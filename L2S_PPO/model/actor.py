...
class WindowEncoder(nn.Module):
    ...
    def _pool_max(self, x, group_idx, num_groups):
        out, _ = scatter_max(x, group_idx, dim=0, dim_size=num_groups)
        # scatter_max 对空 group 会填很小的负值，改成 0
        # 但我们的场景每个 machine 至少有一个 op，所以一般不会出现空 group
        return out
...
class Actor(nn.Module):
    def __init__(self,
        ...
        # ===== PPO critic head =====
        # Consumes the per-graph embedding (2H from GIN+DGHAN) and produces a
        # scalar V(s) for each batch item.
    ...
    def forward(self, batch_states, batch_window_states, feasible_actions,
                chosen_actions=None, return_value=False):
        # ===== 1) global embedding =====
        ...
        entropies = [torch.zeros((), device=device) for _ in range(batch_size)]

        # State value V(s) from the per-graph embedding — always available, even
        # when no candidate window exists for any batch item.
        values = self.value_head(graph_embed).squeeze(-1).unsqueeze(-1)  # [B, 1]

        if num_windows == 0:
            log_prob = torch.stack(log_probs, dim=0).unsqueeze(-1)
            entropy = torch.stack(entropies, dim=0).unsqueeze(-1)
            if chosen_actions is not None or return_value:
                return sampled_actions, log_prob, entropy, values
        ...
        for b in range(batch_size):
            ...
            if chosen_actions is None:
                ...
                # Recover the candidate index that produced the stored anchor
                # action. The candidate set at this state is deterministic, so
                # it matches the one used at rollout time.
                target_action = int(chosen_actions[b])
                cand_actions = [action_per_window[w] for w in cand_idx]
                try:
                    pos = cand_actions.index(target_action)
                except ValueError:
                    # Fallback: action no longer feasible (shouldn't happen if
                    # PPO updates re-use the rollout states verbatim).
                    import warnings
                    warnings.warn(
                        f"PPO evaluate: stored action {target_action} not in current "
                        f"candidates {cand_actions} for batch {b}. Skipping.",
                        RuntimeWarning,
                    )
                    continue
                idx = torch.tensor(pos, dtype=torch.long, device=device)

            sampled_actions[b] = action_per_window[cand_idx[int(idx.item())]]
            log_probs[b] = dist.log_prob(idx)
            entropies[b] = dist.entropy()
