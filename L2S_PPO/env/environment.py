...
class JsspWindow:
    def __init__(self, n_job, n_mch, low, high,
        ...
        # initial_objs: makespan at reset (kept for logging / scaling).
    ...
    def _update_reward_stats(self, reward):
        rewards_np = reward.detach().cpu().numpy().flatten()
        for r in rewards_np:
            self.reward_running_count += 1
            delta = r - self.reward_running_mean
            self.reward_running_mean += delta / self.reward_running_count
            delta2 = r - self.reward_running_mean
            self.reward_running_var += delta * delta2

    @property
    def reward_running_std(self):
        if self.reward_running_count > 1:
            return float(np.sqrt(self.reward_running_var / (self.reward_running_count - 1)))
        return 1.0
    ...
    def step(self, actions, device, plot=False):
        ...
        # ===== reward: raw difference between makespans =====
        # PPO variant: reward = prev_obj - current_obj (positive when makespan
        # improves, zero on stagnation). The running best-so-far is still
        # tracked for evaluation/comparability with the REINFORCE baseline.
        prev_obj = self.current_objs
        prev_best = self.best_objs
        new_best = torch.minimum(prev_best, makespan)

        # environment.py step 里
        reward_raw = (prev_obj - makespan).to(torch.float)
        init = self.initial_objs.to(reward_raw.dtype).clamp(min=1.0)
        reward = reward_raw / init
    ...
    def feasible_actions(self, device):
        ...
        for G, instance, tabu_list, start_times, orders, adj_mat_mc in zip(
            ...
            if len(candidate_actions) > 0:
                ...
                # ----- 去重：用 op_ids 作为 key，保留每个唯一窗口的第一个 action -----
                seen_op_ids = []
                unique_states = []
                unique_actions = []
                for state, action in zip(window_states, candidate_actions):
                    op_ids_t = state['op_ids']
                    # 在已见过的 op_ids 里找完全相同的
                    duplicate = False
                    for seen in seen_op_ids:
                        if seen.shape == op_ids_t.shape and torch.equal(seen, op_ids_t):
                            duplicate = True
                            break
                    if not duplicate:
                        seen_op_ids.append(op_ids_t)
                        unique_states.append(state)
                        unique_actions.append(action)

                if len(unique_states) > 0:
                    actions.append(unique_actions)
                    batch_window_states.append(unique_states)
                    feasible_actions_flag.append(True)
                else:
                    actions.append([])
                    batch_window_states.append([])
