...
class PDRRollout:
    ...
    def reset(self) -> PDRState:
        ...
        self.cur_time = 0
        self.mch_free = np.ones(self.graph.num_machines, dtype=np.int64)
    ...
    def step(self, j: int, i: int) -> PDRState:
        ...
        if not self.finished():
            job_av = True
            free = np.any(self.mch_free == 1)
            if free:
                # check if there is any job available for one of the free machines
                job_av = np.any(self.op_time_left[self.mch_free == 1] < INF)
    ...
    def _get_state(self) -> PDRState:
        ...
        return PDRState(
            ...
            job_av=np.any(self.op_time_left[self.mch_free == 1] < INF, -1),
