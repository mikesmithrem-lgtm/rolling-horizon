...
def load_raw(path: str = './bachmarks/', file_count=50):
    """
...
def group_test_data(path: str, n_j=15, n_m=15, file_count=50):
    output_file = f"./test_dataset/dataset_{n_j}x{n_m}_test"
    ...
    for file in os.listdir(path):
        ...
        f_path = os.path.join(path, file)
        shutil.copy(f_path, output_file)
        file_total += 1
        if file_total >= file_count:
            break
...
if __name__ == '__main__':
    n_j = 6
    n_m = 6
    path = "./co_datasets/jsp/dataset_6x6_test"
    group_test_data(path, n_j=n_j, n_m=n_m)